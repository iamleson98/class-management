...
export function ChannelCallToast({ channelId }: { channelId: string }) {
        ...
        useEffect(() => {
                ...
                const load = async () => {
                        try {
                                ...
                                if (!res.ok) {
                                        // 404 = the call no longer exists server-side: it ended
                                        // while the call_end broadcast was missed (e.g. the server
                                        // restarted and lost the in-memory registry). Drop the
                                        // stale entry so the toast stops advertising a dead call
                                        // instead of polling it forever.
                                        if (res.status === 404 && !cancelled) {
                                                useCallsStore.getState().markActiveCall(channelId, null)
                                        }
                                        return
                                }
        ...
        }, [active?.callId, inThisCall, channelId])
