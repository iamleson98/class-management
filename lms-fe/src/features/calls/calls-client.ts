...
/** How long to wait for the server's join ack before declaring a timeout. */
...
class CallsClient {
        ...
        /** Watchdog for the join ack: no ack within JOIN_ACK_TIMEOUT_MS → error. */
        private joinAckTimer: number | null = null
        private joinedAt = 0
        ...
        /**
         * Watchdog for the JOIN ACK itself. The server only replies to a join it
         * accepted; a silently dropped action (websocket down mid-send, join
         * dropped by the permission gate, server restart) otherwise leaves the
         * call screen stuck on "connecting…" forever with no error anywhere.
         * Timing out here surfaces the standard timeout modal with a working
         * re-join button instead.
         */
        private startJoinAckTimeout(): void {
                this.clearJoinAckTimeout()
                this.joinAckTimer = window.setTimeout(() => {
                        this.joinAckTimer = null
                        if (this.connecting && !this.joined) {
                                this.disconnect(rtcPeerTimeoutErr, 'rtc-timeout')
                        }
                }, JOIN_ACK_TIMEOUT_MS)
        }

        private clearJoinAckTimeout(): void {
                if (this.joinAckTimer !== null) {
                        window.clearTimeout(this.joinAckTimer)
                        this.joinAckTimer = null
                }
        }
         ...
         * RTC timeouts and unrecoverable peer failures. The channel is
         * captured before leave() (the teardown reset nulls it) so the modal
         * can offer re-join.
         */
        private disconnect(err: Error, kind: 'rtc-timeout' | 'rtc-failed'): void {
                const channel = this.channelId
                this.leave()
                useCallsStore.getState().setError({ message: err.message, kind, channelId: channel || null })
        }
