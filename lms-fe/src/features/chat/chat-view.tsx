...
export default function ChatView() {
  ...
  useQuery({
    ...
    select: (calls) => {
      // Mirror results into the calls store (idempotent).
      const active = useCallsStore.getState().activeCalls
      for (const c of calls) {
        if (!active[c.channelId]) markActiveCall(c.channelId, { callId: c.callId, startAt: c.startAt })
      }
      return calls
    },
  })
