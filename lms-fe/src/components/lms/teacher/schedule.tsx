...
function statusBadge(status: string, t: (key: string, fallback?: string) => string) {
  if (status === 'COMPLETED') return <Badge className="bg-teal-500/15 text-teal-700 dark:text-teal-400 border-teal-500/20 text-[10px]">{t('teacher.schedule.statusCompleted', 'Hoàn thành')}</Badge>
  if (status === 'CANCELLED') return <Badge variant="destructive" className="text-[10px]">{t('teacher.schedule.statusCancelled', 'Đã hủy')}</Badge>
  return <Badge variant="outline" className="text-[10px]">{t('teacher.schedule.statusUpcoming', 'Sắp tới')}</Badge>
}
...
export default function TeacherSchedule() {
  ...
  const handlePrev = () => { setDirection(-1); changeMonth(-1) }
  const handleNext = () => { setDirection(1); changeMonth(1) }

  if (isLoading) {
    return (
      <div className="space-y-6">
        <PageHeader title={t('teacher.schedule.title', 'Thời khóa biểu')} description={t('teacher.schedule.description', 'Lịch dạy của bạn')} icon={CalendarDays} accentColor="teal" />
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6">
      <PageHeader title="Thời khóa biểu" description="Lịch dạy của bạn" icon={CalendarDays} accentColor="teal" />

      {/* Stats */}
      <div className="grid grid-cols-3 gap-3">
        <Card className="rounded-xl border-teal-100 dark:border-teal-900/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-teal-600 dark:text-teal-400">{totalSessions}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{t('teacher.schedule.totalSessions', 'Tổng buổi')}</div>
          </CardContent>
        </Card>
        <Card className="rounded-xl border-teal-100 dark:border-teal-900/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-teal-600 dark:text-teal-400">{completedSessions}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{t('teacher.schedule.completed', 'Hoàn thành')}</div>
          </CardContent>
        </Card>
        <Card className="rounded-xl border-teal-100 dark:border-teal-900/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-teal-600 dark:text-teal-400">{upcomingSessions}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{t('teacher.schedule.upcoming', 'Sắp tới')}</div>
          </CardContent>
        </Card>
      </div>
                  ...
                  <TooltipProvider key={dayStr} delayDuration={100}>
                    <div
                      className={cn(
                        ...
                        isTodayDate && 'ring-2 ring-teal-500/40 border-teal-500/30',
                      ...
                      <div className="flex items-center justify-between mb-0.5 px-1">
                        <span
                          className={cn(
                            ...
                            isTodayDate && 'text-teal-700 dark:text-teal-400 font-bold',
                          )}
                        >
                          {format(day, 'd')}
                        </span>
                        {isTodayDate && (
                          <span className="text-[9px] font-semibold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30 px-1.5 py-0.5 rounded-full">
                            {t('teacher.schedule.today', 'Hôm nay')}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="hidden md:block space-y-0.5">
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  {session.room && (
                                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                      ...
                                      <span>{t('teacher.schedule.room', 'Phòng')} {session.room}</span>
                                    </div>
                                  )}
                                  <div className="pt-0.5">{statusBadge(session.status, t)}</div>
                                </div>
                              </TooltipContent>
                            </Tooltip>
                          )
                        ...
                        {daySessions.length > MAX_MINI_CARDS && (
                          <div className="text-[10px] text-muted-foreground px-1.5 font-medium">
                            +{daySessions.length - MAX_MINI_CARDS} {t('teacher.schedule.more', 'hơn')}
                          </div>
                        )}
                      </div>
      ...
      {sessions.length === 0 && !isLoading && (
        <EmptyState
          icon={CalendarDays}
          title={t('teacher.schedule.noSessionsTitle', 'Không có buổi dạy')}
          description={t('teacher.schedule.noSessionsDesc', 'Chưa có buổi học được lên lịch trong tháng này.')}
        />
      )}
    </motion.div>
  )
