...
function statusBadge(status: string) {
  if (status === 'COMPLETED') return <Badge className="bg-cyan-500/15 text-cyan-700 dark:text-cyan-400 border-cyan-500/20 text-[10px]">Hoàn thành</Badge>
...
export default function StudentSchedule() {
  ...
  const studentRecord = useMemo(() => {
    if (!students || !authUser?.id) return null
    return students.find((s: any) => s.userId === authUser.id) || null
  }, [students, authUser])
  ...
  if (isLoading || isLoadingStudents) {
    return (
      <div className="space-y-6">
        <PageHeader title={t('student.schedule.title', 'Thời khóa biểu')} description={t('student.schedule.description', 'Lịch học của bạn')} icon={CalendarDays} accentColor="amber" />
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6">
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-3">
        <Card className="rounded-xl border-cyan-100 dark:border-cyan-900/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-cyan-600 dark:text-cyan-400">{totalSessions}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{t('student.schedule.totalSessions', 'Tổng buổi')}</div>
          </CardContent>
        </Card>
        <Card className="rounded-xl border-cyan-100 dark:border-cyan-900/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-cyan-600 dark:text-cyan-400">{completedSessions}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{t('student.schedule.completed', 'Hoàn thành')}</div>
          </CardContent>
        </Card>
        <Card className="rounded-xl border-cyan-100 dark:border-cyan-900/30">
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-cyan-600 dark:text-cyan-400">{upcomingSessions}</div>
            <div className="text-xs text-muted-foreground mt-0.5">{t('student.schedule.upcoming', 'Sắp tới')}</div>
          </CardContent>
        </Card>
      </div>
                  ...
                  <TooltipProvider key={dayStr} delayDuration={100}>
                    <div
                      className={cn(
                        ...
                        isTodayDate && 'ring-2 ring-cyan-500/40 border-cyan-500/30',
                      ...
                      <div className="flex items-center justify-between mb-0.5 px-1">
                        <span
                          className={cn(
                            ...
                            isTodayDate && 'text-cyan-700 dark:text-cyan-400 font-bold',
                          )}
                        >
                          {format(day, 'd')}
                        </span>
                        {isTodayDate && (
                          <span className="text-[9px] font-semibold text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-950/30 px-1.5 py-0.5 rounded-full">
                            {t('student.schedule.today', 'Hôm nay')}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="hidden md:block space-y-0.5">
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  {teacher && (
                                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                      ...
                                      <span>{t('student.schedule.teacherAbbr', 'GV')}: {teacher.name}</span>
                                    </div>
                                  )}
                                  {session.room && (
                                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                      <MapPin className="h-3 w-3" />
                                      <span>{t('student.schedule.room', 'Phòng')} {session.room}</span>
                                    </div>
                                  )}
                        ...
                        {daySessions.length > MAX_MINI_CARDS && (
                          <div className="text-[10px] text-muted-foreground px-1.5 font-medium">
                            +{daySessions.length - MAX_MINI_CARDS} {t('student.schedule.more', 'hơn')}
                          </div>
                        )}
                      </div>
      ...
      {/* Empty state */}
      {sessions.length === 0 && !isLoading && !!studentRecord && (
        <EmptyState
          icon={CalendarDays}
          title={t('student.schedule.noSessions', 'Không có buổi học')}
          description={t('student.schedule.noSessionsDesc', 'Chưa có buổi học nào được lên lịch trong tháng này.')}
        />
      )}
    </motion.div>
  )
