...
import (
	...
	"github.com/iamleson98/sitename/server/public/utils/timeutils"
)
...
const (
	...
	// LMS Role IDs
	RoleLmsSuperAdminRoleId = "lms_super_admin"
	RoleLmsAdminRoleId      = "lms_admin"
	RoleLmsCounselorRoleId  = "lms_counselor"
	RoleLmsTeacherRoleId    = "lms_teacher"
	RoleLmsAccountantRoleId = "lms_accountant"
	RoleLmsMarketingRoleId  = "lms_marketing"
	RoleLmsParentRoleId     = "lms_parent"
	RoleLmsStudentRoleId    = "lms_student"
...
func MakeDefaultLMSRoles() map[string]*Role {
	...
	roles[RoleLmsTeacherRoleId] = &Role{
		...
		Permissions: []string{
			...
			// ManageStudents grants the class-roster reads the attendance UI needs
			// (POST /lms/students with class_id) — the LMS API has no separate
	...
	roles[RoleLmsMarketingRoleId] = &Role{
		...
		Permissions: []string{
			...
			// Leads power the marketing dashboard (conversion funnel).
			PermissionLmsManageLeads.Id,
