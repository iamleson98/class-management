...
import (
	...
	"github.com/iamleson98/sitename/server/v8/channels/app/imaging"
	"github.com/iamleson98/sitename/server/v8/channels/store"
	"github.com/iamleson98/sitename/server/v8/channels/utils"
	"github.com/iamleson98/sitename/server/v8/platform/services/docextractor"
...
func (a *App) CheckMandatoryS3Fields(settings *model.FileSettings) *model.AppError {
	...
	if a.Config().FeatureFlags.CloudDedicatedExportUI && a.Config().FileSettings.DedicatedExportStore != nil && *a.Config().FileSettings.DedicatedExportStore {
...
func (a *App) TestFileStoreConnectionWithConfig(cfg *model.FileSettings) *model.AppError {
	...
	if a.Config().FeatureFlags.CloudDedicatedExportUI && a.Config().FileSettings.DedicatedExportStore != nil && *a.Config().FileSettings.DedicatedExportStore {
		...
		backend, err = filestore.NewFileBackend(filestore.NewExportFileBackendSettingsFromConfig(cfg, true, allowInsecure))
	} else {
		backend, err = filestore.NewFileBackend(filestore.NewFileBackendSettingsFromConfig(cfg, true, insecure != nil && *insecure))
	}
	if err != nil {
