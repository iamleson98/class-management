...
func (s *Server) doLMSRolesCreationMigration() error {
	...
	for roleID, role := range roles {
		...
		if err != nil && !errors.As(err, &nfErr) {
			multiErr = multierror.Append(multiErr, fmt.Errorf("failed to check for LMS role %q: %w", roleID, err))
			continue
		}
		if existing != nil {
		...
		if _, err := s.Store().Role().Save(role); err != nil {
			multiErr = multierror.Append(multiErr, fmt.Errorf("failed to create LMS role %q: %w", roleID, err))
		}
	}
...
func (s *Server) doAppMigrations() {
	...
	m1 := []migration{
		...
		{"LMS Roles Creation Migration", s.doLMSRolesCreationMigration},
