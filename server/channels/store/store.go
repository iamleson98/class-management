...
type Store interface {
	...
	GetMasterExecuter() boil.Executor
	GetReplicaExecuter() boil.Executor
	...
	Branch() BranchStore
	Course() CourseStore
	CourseLesson() CourseLessonStore
	Class() ClassStore
	StudentClass() StudentClassStore
	LMSSession() LMSSessionStore
	Attendance() AttendanceStore
	Lead() LeadStore
	LeadActivity() LeadActivityStore
	FeePackage() FeePackageStore
	Tuition() TuitionStore
	Payment() PaymentStore
	FeeRefund() FeeRefundStore
	AdditionalFee() AdditionalFeeStore
	PostCategory() PostCategoryStore
	BlogPost() BlogPostStore
	WeeklyReview() WeeklyReviewStore
	Homework() HomeworkStore
	Submission() SubmissionStore
	ClassMedia() ClassMediaStore
	Task() TaskStore
	Banner() BannerStore
...
func IsUniqueConstraintError(err error, indexNames []string) bool {
	if err == nil {
		return false
	}
	...
	// pgx/pgconn and other drivers exposing SQLSTATE.
	...
	if !unique {
		...
		if errors.As(errors.Cause(err), &stateErr) {
			unique = stateErr.SQLState() == "23505"
		}
	}
