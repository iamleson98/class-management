...
import (
	...
	sqlUtils "github.com/iamleson98/sitename/server/public/utils/sql"
	...
	"github.com/iamleson98/sitename/server/v8/channels/db"
	"github.com/iamleson98/sitename/server/v8/channels/store"
	lmsstore "github.com/iamleson98/sitename/server/v8/channels/store/sqlstore/lms_store"
...
type SqlStoreStores struct {
	...
	courseLesson  store.CourseLessonStore
	class         store.ClassStore
	studentClass  store.StudentClassStore
	lmsSession    store.LMSSessionStore
	attendance    store.AttendanceStore
	lead          store.LeadStore
	leadActivity  store.LeadActivityStore
	feePackage    store.FeePackageStore
	tuition       store.TuitionStore
	payment       store.PaymentStore
	feeRefund     store.FeeRefundStore
	additionalFee store.AdditionalFeeStore
	postCategory  store.PostCategoryStore
	blogPost      store.BlogPostStore
	weeklyReview  store.WeeklyReviewStore
	homework      store.HomeworkStore
	submission    store.SubmissionStore
	classMedia    store.ClassMediaStore
...
type SqlStore struct {
	...
	// See https://github.com/iamleson98/sitename/server/v8/channels/pull/7281
	rrCounter int64
	srCounter int64
...
func New(settings model.SqlSettings, logger mlog.LoggerIFace, metrics einterfaces.MetricsInterface, options ...Option) (*SqlStore, error) {
	...
	// LMS stores
	store.stores.branch = lmsstore.NewSqlBranchStore(store)
	store.stores.course = lmsstore.NewSqlCourseStore(store)
	store.stores.courseLesson = lmsstore.NewSqlCourseLessonStore(store)
	store.stores.class = lmsstore.NewSqlClassStore(store)
	store.stores.studentClass = lmsstore.NewSqlStudentClassStore(store)
	store.stores.lmsSession = lmsstore.NewSqlLMSSessionStore(store)
	store.stores.attendance = lmsstore.NewSqlAttendanceStore(store)
	store.stores.lead = lmsstore.NewSqlLeadStore(store)
	store.stores.leadActivity = lmsstore.NewSqlLeadActivityStore(store)
	store.stores.feePackage = lmsstore.NewSqlFeePackageStore(store)
	store.stores.tuition = lmsstore.NewSqlTuitionStore(store)
	store.stores.payment = lmsstore.NewSqlPaymentStore(store)
	store.stores.feeRefund = lmsstore.NewSqlFeeRefundStore(store)
	store.stores.additionalFee = lmsstore.NewSqlAdditionalFeeStore(store)
	store.stores.postCategory = lmsstore.NewSqlPostCategoryStore(store)
	store.stores.blogPost = lmsstore.NewSqlBlogPostStore(store)
	store.stores.weeklyReview = lmsstore.NewSqlWeeklyReviewStore(store)
	store.stores.homework = lmsstore.NewSqlHomeworkStore(store)
	store.stores.submission = lmsstore.NewSqlSubmissionStore(store)
	store.stores.classMedia = lmsstore.NewSqlClassMediaStore(store)
	store.stores.task = lmsstore.NewSqlTaskStore(store)
	store.stores.banner = lmsstore.NewSqlBannerStore(store)
...
// GetMasterExecuter implements [store.Store].
func (ss *SqlStore) GetMasterExecuter() boil.Executor {
	return ss.GetMaster()
}

// GetReplicaExecuter implements [store.Store].
func (ss *SqlStore) GetReplicaExecuter() boil.Executor {
...
func (ss *SqlStore) Branch() store.BranchStore {
	return ss.stores.branch
}

func (ss *SqlStore) Course() store.CourseStore {
	return ss.stores.course
}

func (ss *SqlStore) CourseLesson() store.CourseLessonStore {
	return ss.stores.courseLesson
}
...
func (ss *SqlStore) StudentClass() store.StudentClassStore {
	return ss.stores.studentClass
}

func (ss *SqlStore) LMSSession() store.LMSSessionStore {
	return ss.stores.lmsSession
}

func (ss *SqlStore) Attendance() store.AttendanceStore {
	return ss.stores.attendance
}

func (ss *SqlStore) Lead() store.LeadStore {
	return ss.stores.lead
}

func (ss *SqlStore) LeadActivity() store.LeadActivityStore {
	return ss.stores.leadActivity
}

func (ss *SqlStore) FeePackage() store.FeePackageStore {
	return ss.stores.feePackage
}

func (ss *SqlStore) Tuition() store.TuitionStore {
	return ss.stores.tuition
}
...
func (ss *SqlStore) FeeRefund() store.FeeRefundStore {
	return ss.stores.feeRefund
}

func (ss *SqlStore) AdditionalFee() store.AdditionalFeeStore {
	return ss.stores.additionalFee
}

func (ss *SqlStore) PostCategory() store.PostCategoryStore {
	return ss.stores.postCategory
}

func (ss *SqlStore) BlogPost() store.BlogPostStore {
	return ss.stores.blogPost
}

func (ss *SqlStore) WeeklyReview() store.WeeklyReviewStore {
	return ss.stores.weeklyReview
}

func (ss *SqlStore) Homework() store.HomeworkStore {
	return ss.stores.homework
}

func (ss *SqlStore) Submission() store.SubmissionStore {
	return ss.stores.submission
}

func (ss *SqlStore) ClassMedia() store.ClassMediaStore {
	return ss.stores.classMedia
}

func (ss *SqlStore) Task() store.TaskStore {
	return ss.stores.task
}

func (ss *SqlStore) Banner() store.BannerStore {
	return ss.stores.banner
}

func (ss *SqlStore) Notification() store.NotificationStore {
	return ss.stores.notification
}

func (ss *SqlStore) Material() store.MaterialStore {
	return ss.stores.material
}
