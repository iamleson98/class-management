...
func (w *Web) InitStatic() {
	if *w.srv.Config().ServiceSettings.WebserverMode != "disabled" {
		...
		w.MainRouter.Handle("/static/plugins/", pluginHandler)
		w.MainRouter.Handle("/static/", staticHandler)
		...
		w.MainRouter.Method(http.MethodGet, "/{anything:.*}", w.NewStaticHandler(root))
		w.MainRouter.Method(http.MethodHead, "/{anything:.*}", w.NewStaticHandler(root))
