...
func (w *Web) InitOAuth() {
	...
	w.MainRouter.Method(http.MethodGet, model.OAuthMetadataEndpoint, w.APIHandlerTrustRequester(getAuthorizationServerMetadata))

	// API version independent OAuth 2.0 as a service provider endpoints
	w.MainRouter.Method(http.MethodGet, model.OAuthAuthorizeEndpoint, w.APIHandlerTrustRequester(authorizeOAuthPage))
	w.MainRouter.Method(http.MethodPost, model.OAuthAuthorizeEndpoint, w.APISessionRequired(authorizeOAuthApp))
	w.MainRouter.Method(http.MethodPost, model.OAuthDeauthorizeEndpoint, w.APISessionRequired(deauthorizeOAuthApp))
	w.MainRouter.Method(http.MethodPost, model.OAuthAccessTokenEndpoint, w.APIHandlerTrustRequester(getAccessToken))

	// API version independent OAuth as a client endpoints
	w.MainRouter.Method(http.MethodGet, "/oauth/{service:[A-Za-z0-9]+}/complete", w.APIHandler(completeOAuth))
	w.MainRouter.Method(http.MethodGet, "/oauth/{service:[A-Za-z0-9]+}/login", w.APIHandler(loginWithOAuth))
	w.MainRouter.Method(http.MethodGet, "/oauth/{service:[A-Za-z0-9]+}/mobile_login", w.APIHandler(mobileLoginWithOAuth))
	w.MainRouter.Method(http.MethodGet, "/oauth/{service:[A-Za-z0-9]+}/signup", w.APIHandler(signupWithOAuth))

	// Intune MAM authentication endpoint
	w.MainRouter.Method(http.MethodPost, "/oauth/intune", w.APIHandler(loginByIntune))

	// Old endpoints for backwards compatibility, needed to not break SSO for any old setups
	w.MainRouter.Method(http.MethodGet, "/api/v3/oauth/{service:[A-Za-z0-9]+}/complete", w.APIHandler(completeOAuth))
	w.MainRouter.Method(http.MethodGet, "/signup/{service:[A-Za-z0-9]+}/complete", w.APIHandler(completeOAuth))
	w.MainRouter.Method(http.MethodGet, "/login/{service:[A-Za-z0-9]+}/complete", w.APIHandler(completeOAuth))
	w.MainRouter.Method(http.MethodGet, "/api/v4/oauth_test", w.APISessionRequired(testHandler))
}
...
func completeOAuth(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	service := ""
	if s, ok := c.Params["service"].(string); ok {
		service = s
	}
...
func loginWithOAuth(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	auditRec.AddMeta("service", service)
	defer c.LogAuditRec(auditRec)
	...
	authURL, err := c.App.GetOAuthLoginEndpoint(c.AppContext, w, r, service, model.OAuthActionLogin, redirectURL, loginHint, false, desktopToken, tokenID, inviteId)
	if err != nil {
		c.Err = err
		return
	}
...
func mobileLoginWithOAuth(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	authURL, err := c.App.GetOAuthLoginEndpoint(c.AppContext, w, r, service, model.OAuthActionMobile, redirectURL, "", true, "", tokenID, inviteId)
	if err != nil {
		c.Err = err
		return
	}
...
func signupWithOAuth(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	authURL, err := c.App.GetOAuthSignupEndpoint(c.AppContext, w, r, service, desktopToken, tokenID, inviteId)
	if err != nil {
		c.Err = err
		return
	}
