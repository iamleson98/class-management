...
type RTCDClient interface {
	Connect() error
	Connected() bool
	// Heal force-rebuilds a dead control connection (throttled). Returns
	// whether the client is connected afterwards.
	Heal() bool
...
type rtcdClientManager struct {
	...
	// inject a fake without touching the network. The manager instance is
	// passed explicitly: the factory is invoked DURING construction (before
	// CallService.rtcd is assigned), so a factory that reaches back through
	// the service's manager field would always see nil there.
	newClient func(m *rtcdClientManager, rtcdURL, host string) (RTCDClient, error)
...
func newRTCDClientManager(rtcdURL string, log mlog.LoggerIFace, kv KVStore, newClient func(m *rtcdClientManager, rtcdURL, host string) (RTCDClient, error), onMessage func(host string, msg rtcd.ClientMessage)) (*rtcdClientManager, error) {
	m := &rtcdClientManager{
		log:       log,
		store:     kv,
	...
	for _, ip := range ips {
		client, err := m.newClient(m, rtcdURL, ip.String())
		if err != nil {
			m.closeAllHosts()
	...
	for {
		...
		case <-ticker.C:
			...
			for ip := range ipsMap {
				...
				client, err := m.newClient(m, m.rtcdURL, ip)
				if err != nil {
...
// to spread load across the SFU pool. When NO host is healthy the unflagged
// ones get one forced Heal each (rebuild + re-register + reconnect) before
// giving up — a dead control socket (rtcd restarted, lost auth store, wedged
// reconnect loop) otherwise rejects every new call until a server restart,
// which is exactly the "call screen opens then immediately quits" failure
// class reported in production.
func (m *rtcdClientManager) GetHostForNewCall() (string, error) {
	if host, ok := m.pickHealthyHost(); ok {
		return host, nil
	}

	// No healthy host: heal the unflagged candidates (I/O — outside the lock).
	m.mut.RLock()
	candidates := make([]RTCDClient, 0, len(m.hosts))
	for _, h := range m.hosts {
		if !h.isFlagged() && !h.client.Connected() {
			candidates = append(candidates, h.client)
		}
	}
	m.mut.RUnlock()
	...
	// A heal may have brought one back.
	if host, ok := m.pickHealthyHost(); ok {
		return host, nil
	}
...
// pickHealthyHost picks a random connected, unflagged host (taking the
// manager read lock itself).
func (m *rtcdClientManager) pickHealthyHost() (string, bool) {
	m.mut.RLock()
	defer m.mut.RUnlock()
	...
	return net.JoinHostPort(h.ip, m.rtcdPort), true
}
