...
//
// Length contract: channel IDs are 26 characters (channels.id is varchar(26)),
// so "ch:" + channelID is 29 characters — within the varchar(32) width of
// calls.id and the call_*.callid columns (migration 000161). The clamp below is
// defensive: a malformed input longer than a real channel ID must never derive
// a call ID that cannot be persisted.
func callIDForChannel(channelID string) string {
        if len(channelID) > 26 {
                channelID = channelID[:26]
        }
        return "ch:" + channelID
}
