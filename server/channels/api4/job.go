...
import (
	...
	pweb "github.com/iamleson98/sitename/server/v8/platform/shared/web"
)

func (api *API) InitJob() {
	api.BaseRoutes.Jobs.Method(http.MethodGet, "/", api.APISessionRequired(getJobs))
	api.BaseRoutes.Jobs.Method(http.MethodPost, "/", api.APISessionRequired(createJob))
	api.BaseRoutes.Jobs.Method(http.MethodGet, "/{job_id:[A-Za-z0-9]+}", api.APISessionRequired(getJob))
	api.BaseRoutes.Jobs.Method(http.MethodGet, "/{job_id:[A-Za-z0-9]+}/download", api.APISessionRequiredTrustRequester(downloadJob))
	api.BaseRoutes.Jobs.Method(http.MethodPost, "/{job_id:[A-Za-z0-9]+}/cancel", api.APISessionRequired(cancelJob))
	api.BaseRoutes.Jobs.Method(http.MethodGet, "/type/{job_type:[A-Za-z0-9_-]+}", api.APISessionRequired(getJobsByType))
	api.BaseRoutes.Jobs.Method(http.MethodPatch, "/{job_id:[A-Za-z0-9]+}/status", api.APISessionRequired(updateJobStatus))
}
...
func downloadJob(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !ok || exportDir == "" || fileName == "/" || fileName == "." {
		...
		pweb.WriteFileResponse(fileName, FileMime, 0, time.UnixMilli(job.LastActivityAt), *c.App.Config().ServiceSettings.WebserverMode, fileReader, true, w, r)
		return
	}
	...
	if err := pweb.WriteStreamResponse(w, zipReader, fileName, FileMime, true); err != nil {
		c.Err = model.NewAppError("unableToDownloadJob", "api.job.unable_to_download_job", nil,
...
func getJobs(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if status == "" {
		jobs, appErr = c.App.GetJobsByTypesPage(c.AppContext, validJobTypes, c.Params["page"].(int), c.Params["per_page"].(int))
	} else {
		jobs, appErr = c.App.GetJobsByTypesAndStatuses(c.AppContext, validJobTypes, []string{status}, c.Params["page"].(int), c.Params["per_page"].(int))
	}
...
func getJobsByType(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	jobs, appErr := c.App.GetJobsByTypePage(c.AppContext, jobTypeStr, c.Params["page"].(int), c.Params["per_page"].(int))
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func cancelJob(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	defer c.LogAuditRec(auditRec)
	model.AddEventParameterToAuditRec(auditRec, "job_id", jobIdStr)
	...
	if err := c.App.CancelJob(c.AppContext, jobIdStr); err != nil {
		c.Err = err
		return
	}
