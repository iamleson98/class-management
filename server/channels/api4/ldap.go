...
func (api *API) InitLdap() {
	api.BaseRoutes.LDAP.Method(http.MethodPost, "/sync", api.APISessionRequired(syncLdap))
	api.BaseRoutes.LDAP.Method(http.MethodPost, "/test", api.APISessionRequired(testLdap))
	api.BaseRoutes.LDAP.Method(http.MethodPost, "/test_connection", api.APISessionRequired(testLdapConnection))
	api.BaseRoutes.LDAP.Method(http.MethodPost, "/test_diagnostics", api.APISessionRequired(testLdapDiagnostics))

	api.BaseRoutes.LDAP.Method(http.MethodPost, "/migrateid", api.APISessionRequired(migrateIDLdap))

	// GET /api/v4/ldap/groups?page=0&per_page=1000
	api.BaseRoutes.LDAP.Method(http.MethodGet, "/groups", api.APISessionRequired(getLdapGroups))

	// POST /api/v4/ldap/groups/:remote_id/link
	api.BaseRoutes.LDAP.Method(http.MethodDelete, `/groups/{remote_id}/link`, api.APISessionRequired(unlinkLdapGroup))

	api.BaseRoutes.LDAP.Method(http.MethodPost, "/certificate/public", api.APISessionRequired(addLdapPublicCertificate, handlerParamFileAPI))
	api.BaseRoutes.LDAP.Method(http.MethodPost, "/certificate/private", api.APISessionRequired(addLdapPrivateCertificate, handlerParamFileAPI))

	api.BaseRoutes.LDAP.Method(http.MethodDelete, "/certificate/public", api.APISessionRequired(removeLdapPublicCertificate))
	api.BaseRoutes.LDAP.Method(http.MethodDelete, "/certificate/private", api.APISessionRequired(removeLdapPrivateCertificate))

	api.BaseRoutes.LDAP.Method(http.MethodPost, "/users/{user_id}/group_sync_memberships", api.APISessionRequired(addUserToGroupSyncables))
}
...
func getLdapGroups(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if c.Params["is_linked"] != nil {
		opts.IsLinked = model.NewPointer(c.Params["is_linked"].(bool))
	}
	if c.Params["is_configured"] != nil {
		opts.IsConfigured = model.NewPointer(c.Params["is_configured"].(bool))
	}

	groups, total, appErr := c.App.GetAllLdapGroupsPage(c.AppContext, c.Params["page"].(int), c.Params["per_page"].(int), opts)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func linkLdapGroup(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	ldapGroup, appErr := c.App.GetLdapGroup(c.AppContext, remoteIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func unlinkLdapGroup(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	group, err := c.App.GetGroupByRemoteID(remoteIdStr, model.GroupSourceLdap)
	if err != nil {
		c.Err = err
		return
	}
