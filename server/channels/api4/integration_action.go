...
func (api *API) InitAction() {
	api.BaseRoutes.Post.Method(http.MethodPost, "/actions/{action_id:[A-Za-z0-9]+}", api.APISessionRequired(doPostAction))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/actions/dialogs/open", api.APIHandler(openDialog))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/actions/dialogs/submit", api.APISessionRequired(submitDialog))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/actions/dialogs/lookup", api.APISessionRequired(lookupDialog))
}
...
func doPostAction(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	resp.TriggerId, appErr = c.App.DoPostActionWithCookie(c.AppContext, postIdStr, actionIdStr, c.AppContext.Session().UserId,
