...
func (api *API) InitBrand() {
	api.BaseRoutes.Brand.Method(http.MethodGet, "/image", api.APIHandlerTrustRequester(getBrandImage))
	api.BaseRoutes.Brand.Method(http.MethodPost, "/image", api.APISessionRequired(uploadBrandImage, handlerParamFileAPI))
	api.BaseRoutes.Brand.Method(http.MethodDelete, "/image", api.APISessionRequired(deleteBrandImage))
}
