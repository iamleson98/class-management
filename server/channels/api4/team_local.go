...
func (api *API) InitTeamLocal() {
	api.BaseRoutes.Teams.Method(http.MethodPost, "/", api.APILocal(localCreateTeam))
	api.BaseRoutes.Teams.Method(http.MethodGet, "/", api.APILocal(getAllTeams))
	api.BaseRoutes.Teams.Method(http.MethodPost, "/search", api.APILocal(searchTeams))
	api.BaseRoutes.Team.Method(http.MethodGet, "/", api.APILocal(getTeam))
	api.BaseRoutes.Team.Method(http.MethodPut, "/", api.APILocal(updateTeam))
	api.BaseRoutes.Team.Method(http.MethodDelete, "/", api.APILocal(localDeleteTeam))
	api.BaseRoutes.Team.Method(http.MethodPost, "/invite/email", api.APILocal(localInviteUsersToTeam))
	api.BaseRoutes.Team.Method(http.MethodPut, "/patch", api.APILocal(patchTeam))
	api.BaseRoutes.Team.Method(http.MethodPut, "/privacy", api.APILocal(updateTeamPrivacy))
	api.BaseRoutes.Team.Method(http.MethodPost, "/restore", api.APILocal(restoreTeam))
	api.BaseRoutes.TeamByName.Method(http.MethodGet, "/", api.APILocal(getTeamByName))
	api.BaseRoutes.TeamMembers.Method(http.MethodPost, "/", api.APILocal(addTeamMember))
	api.BaseRoutes.TeamMember.Method(http.MethodDelete, "/", api.APILocal(removeTeamMember))
}
...
func localInviteUsersToTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	team, err := c.App.Srv().Store().Team().Get(teamIdStr)
	if err != nil {
