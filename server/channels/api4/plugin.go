...
func (api *API) InitPlugin() {
	api.BaseRoutes.Plugins.Method(http.MethodPost, "/", api.APISessionRequired(uploadPlugin, handlerParamFileAPI))
	api.BaseRoutes.Plugins.Method(http.MethodGet, "/", api.APISessionRequired(getPlugins))
	api.BaseRoutes.Plugin.Method(http.MethodDelete, "/", api.APISessionRequired(removePlugin))
	api.BaseRoutes.Plugins.Method(http.MethodPost, "/install_from_url", api.APISessionRequired(installPluginFromURL))
	api.BaseRoutes.Plugins.Method(http.MethodPost, "/marketplace", api.APISessionRequired(installMarketplacePlugin))
	api.BaseRoutes.Plugins.Method(http.MethodGet, "/statuses", api.APISessionRequired(getPluginStatuses))
	api.BaseRoutes.Plugin.Method(http.MethodPost, "/enable", api.APISessionRequired(enablePlugin))
	api.BaseRoutes.Plugin.Method(http.MethodPost, "/disable", api.APISessionRequired(disablePlugin))
	api.BaseRoutes.Plugins.Method(http.MethodGet, "/webapp", api.APIHandler(getWebappPlugins))
	api.BaseRoutes.Plugins.Method(http.MethodGet, "/marketplace", api.APISessionRequired(getMarketplacePlugins))
	api.BaseRoutes.Plugins.Method(http.MethodPost, "/marketplace/first_admin_visit", api.APIHandler(setFirstAdminVisitMarketplaceStatus))
	api.BaseRoutes.Plugins.Method(http.MethodGet, "/marketplace/first_admin_visit", api.APISessionRequired(getFirstAdminVisitMarketplaceStatus))
}
...
func removePlugin(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	defer c.LogAuditRec(auditRec)
	model.AddEventParameterToAuditRec(auditRec, "plugin_id", pluginIdStr)
	...
	err := c.App.Channels().RemovePlugin(pluginIdStr)
	if err != nil {
		c.Err = err
		return
	}
...
func enablePlugin(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := c.App.EnablePlugin(pluginIdStr); err != nil {
		c.Err = err
		return
	}
...
func disablePlugin(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := c.App.DisablePlugin(pluginIdStr); err != nil {
		c.Err = err
		return
	}
