...
import (
	...
	"github.com/iamleson98/sitename/server/v8/platform/services/upgrader"
...
func (api *API) InitSystem() {
	api.BaseRoutes.System.Method(http.MethodGet, "/ping", api.APIHandler(getSystemPing))
	api.BaseRoutes.System.Method(http.MethodGet, "/timezones", api.APISessionRequired(getSupportedTimezones))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/audits", api.APISessionRequired(getAudits))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/notifications/test", api.APISessionRequired(testNotifications))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/email/test", api.APISessionRequired(testEmail))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/site_url/test", api.APISessionRequired(testSiteURL))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/file/s3_test", api.APISessionRequired(testS3))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/database/recycle", api.APISessionRequired(databaseRecycle))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/caches/invalidate", api.APISessionRequired(invalidateCaches))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/logs", api.APISessionRequired(getLogs))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/logs/download", api.APISessionRequired(downloadLogs))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/logs/query", api.APISessionRequired(queryLogs))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/logs", api.APIHandler(postLog))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/analytics/old", api.APISessionRequired(getAnalytics))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/latest_version", api.APISessionRequired(getLatestVersion))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/redirect_location", api.APISessionRequiredTrustRequester(getRedirectLocation))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/notifications/ack", api.APISessionRequired(pushNotificationAck))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/server_busy", api.APISessionRequired(setServerBusy))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/server_busy", api.APISessionRequired(getServerBusyExpires))
	api.BaseRoutes.APIRoot.Method(http.MethodDelete, "/server_busy", api.APISessionRequired(clearServerBusy))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/upgrade_to_enterprise", api.APISessionRequired(upgradeToEnterprise))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/upgrade_to_enterprise/status", api.APISessionRequired(upgradeToEnterpriseStatus))
	api.BaseRoutes.APIRoot.Method(http.MethodGet, "/upgrade_to_enterprise/allowed", api.APISessionRequired(isAllowedToUpgradeToEnterprise))
	api.BaseRoutes.APIRoot.Method(http.MethodPost, "/restart", api.APISessionRequired(restart))
	api.BaseRoutes.System.Method(http.MethodGet, "/notices/{team_id:[A-Za-z0-9]+}", api.APISessionRequired(getProductNotices))
	api.BaseRoutes.System.Method(http.MethodPut, "/notices/view", api.APISessionRequired(updateViewedProductNotices))
	api.BaseRoutes.System.Method(http.MethodGet, "/support_packet", api.APISessionRequired(generateSupportPacket))
	api.BaseRoutes.System.Method(http.MethodGet, "/onboarding/complete", api.APISessionRequired(getOnboarding))
	api.BaseRoutes.System.Method(http.MethodPost, "/onboarding/complete", api.APISessionRequired(completeOnboarding))
	api.BaseRoutes.System.Method(http.MethodGet, "/schema/version", api.APISessionRequired(getAppliedSchemaMigrations))
}
...
func generateSupportPacket(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	now := time.Now()
	outputZipFilename := supportPacketFileName(now, "dxv")
...
func getAudits(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	audits, appErr := c.App.GetAuditsPage(c.AppContext, "", c.Params["page"].(int), c.Params["per_page"].(int))
	if appErr != nil {
		c.Err = appErr
		return
	}
	...
	model.AddEventParameterToAuditRec(auditRec, "audits_per_page", c.Params["logs_per_page"].(int))
...
func queryLogs(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	logs, appErr := c.App.QueryLogs(c.AppContext, c.Params["page"].(int), c.Params["logs_per_page"].(int), logFilter)
	if appErr != nil {
		c.Err = appErr
		return
	}
	...
	auditRec.AddMeta("logs_per_page", c.Params["logs_per_page"].(int))
...
func getLogs(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	lines, appErr := c.App.GetLogs(c.AppContext, c.Params["page"].(int), c.Params["logs_per_page"].(int))
	if appErr != nil {
		c.Err = appErr
		return
	}
	...
	model.AddEventParameterToAuditRec(auditRec, "page", c.Params["page"].(int))
	model.AddEventParameterToAuditRec(auditRec, "logs_per_page", c.Params["logs_per_page"].(int))
...
func getProductNotices(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	notices, appErr := c.App.GetProductNotices(c.AppContext, c.AppContext.Session().UserId, teamIdStr, client, clientVersion, locale)
	if appErr != nil {
		c.Err = appErr
		return
	}
