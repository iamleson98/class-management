...
func (api *API) InitTeam() {
	api.BaseRoutes.Teams.Method(http.MethodPost, "/", api.APISessionRequired(createTeam))
	api.BaseRoutes.Teams.Method(http.MethodGet, "/", api.APISessionRequired(getAllTeams))
	api.BaseRoutes.Teams.Method(http.MethodPut, "/{team_id:[A-Za-z0-9]+}/scheme", api.APISessionRequired(updateTeamScheme))
	api.BaseRoutes.Teams.Method(http.MethodPost, "/search", api.APISessionRequiredDisableWhenBusy(searchTeams))
	api.BaseRoutes.TeamsForUser.Method(http.MethodGet, "/", api.APISessionRequired(getTeamsForUser))
	api.BaseRoutes.TeamsForUser.Method(http.MethodGet, "/unread", api.APISessionRequired(getTeamsUnreadForUser))
	api.BaseRoutes.Team.Method(http.MethodGet, "/", api.APISessionRequired(getTeam))
	api.BaseRoutes.Team.Method(http.MethodPut, "/", api.APISessionRequired(updateTeam))
	api.BaseRoutes.Team.Method(http.MethodDelete, "/", api.APISessionRequired(deleteTeam))
	api.BaseRoutes.Team.Method(http.MethodPut, "/patch", api.APISessionRequired(patchTeam))
	api.BaseRoutes.Team.Method(http.MethodPut, "/privacy", api.APISessionRequired(updateTeamPrivacy))
	api.BaseRoutes.Team.Method(http.MethodPost, "/restore", api.APISessionRequired(restoreTeam))
	api.BaseRoutes.Team.Method(http.MethodPut, "/privacy", api.APISessionRequired(updateTeamPrivacy))
	api.BaseRoutes.Team.Method(http.MethodGet, "/stats", api.APISessionRequired(getTeamStats))
	api.BaseRoutes.Team.Method(http.MethodPost, "/regenerate_invite_id", api.APISessionRequired(regenerateTeamInviteId))
	api.BaseRoutes.Team.Method(http.MethodGet, "/image", api.APISessionRequiredTrustRequester(getTeamIcon))
	api.BaseRoutes.Team.Method(http.MethodPost, "/image", api.APISessionRequired(setTeamIcon, handlerParamFileAPI))
	api.BaseRoutes.Team.Method(http.MethodDelete, "/image", api.APISessionRequired(removeTeamIcon))
	api.BaseRoutes.TeamMembers.Method(http.MethodGet, "/", api.APISessionRequired(getTeamMembers))
	api.BaseRoutes.TeamMembers.Method(http.MethodPost, "/ids", api.APISessionRequired(getTeamMembersByIds))
	api.BaseRoutes.TeamMembersForUser.Method(http.MethodGet, "/", api.APISessionRequired(getTeamMembersForUser))
	api.BaseRoutes.TeamMembers.Method(http.MethodPost, "/", api.APISessionRequired(addTeamMember))
	api.BaseRoutes.Teams.Method(http.MethodPost, "/members/invite", api.APISessionRequired(addUserToTeamFromInvite))
	api.BaseRoutes.TeamMembers.Method(http.MethodPost, "/batch", api.APISessionRequired(addTeamMembers))
	api.BaseRoutes.TeamMember.Method(http.MethodDelete, "/", api.APISessionRequired(removeTeamMember))
	api.BaseRoutes.TeamForUser.Method(http.MethodGet, "/unread", api.APISessionRequired(getTeamUnread))
	api.BaseRoutes.TeamByName.Method(http.MethodGet, "/", api.APISessionRequired(getTeamByName))
	api.BaseRoutes.TeamMember.Method(http.MethodGet, "/", api.APISessionRequired(getTeamMember))
	api.BaseRoutes.TeamByName.Method(http.MethodGet, "/exists", api.APISessionRequired(teamExists))
	api.BaseRoutes.TeamMember.Method(http.MethodPut, "/roles", api.APISessionRequired(updateTeamMemberRoles))
	api.BaseRoutes.TeamMember.Method(http.MethodPut, "/schemeRoles", api.APISessionRequired(updateTeamMemberSchemeRoles))
	api.BaseRoutes.Team.Method(http.MethodPost, "/import", api.APISessionRequired(importTeam))
	api.BaseRoutes.Team.Method(http.MethodPost, "/invite/email", api.APISessionRequired(inviteUsersToTeam))
	api.BaseRoutes.Team.Method(http.MethodPost, "/invite-guests/email", api.APISessionRequired(inviteGuestsToChannels))
	api.BaseRoutes.Teams.Method(http.MethodDelete, "/invites/email", api.APISessionRequired(invalidateAllEmailInvites))
	api.BaseRoutes.Teams.Method(http.MethodGet, "/invite/{invite_id:[A-Za-z0-9]+}", api.APIHandler(getInviteInfo))
	api.BaseRoutes.Teams.Method(http.MethodGet, "/{team_id:[A-Za-z0-9]+}/members_minus_group_members", api.APISessionRequired(teamMembersMinusGroupMembers))
}
...
func getTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	team, err := c.App.GetTeam(teamIdStr)
	if err != nil {
		c.Err = err
		return
	}
...
func getTeamByName(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	team, err := c.App.GetTeamByName(teamNameStr)
	if err != nil {
		c.Err = err
		return
	}
...
func updateTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionManageTeam) {
		c.SetPermissionError(model.PermissionManageTeam)
		return
	}

	oldTeam, err := c.App.GetTeam(teamIdStr)
	if err != nil {
		c.Err = err
		return
	}
	...
	if (team.AllowOpenInvite != oldTeam.AllowOpenInvite || team.AllowedDomains != oldTeam.AllowedDomains) && !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionInviteUser) {
...
func patchTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if (team.AllowOpenInvite != nil || team.AllowedDomains != nil) && !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionInviteUser) {
		c.SetPermissionError(model.PermissionInviteUser)
		return
	}
	...
	patchedTeam, err := c.App.PatchTeam(teamIdStr, &team)
	if err != nil {
		c.Err = err
		return
	}
	...
	if patchedTeam.GroupConstrained != nil && *patchedTeam.GroupConstrained && (oldTeam.GroupConstrained == nil || !*oldTeam.GroupConstrained) {
		c.App.Srv().Go(func() {
			if err := c.App.DeleteGroupConstrainedTeamMemberships(c.AppContext, &teamIdStr); err != nil {
...
func restoreTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	err := c.App.RestoreTeam(teamIdStr)
	if err != nil {
		c.Err = err
		return
	}
...
func updateTeamPrivacy(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionInviteUser) {
		c.SetPermissionError(model.PermissionInviteUser)
		return
	}

	if err := c.App.UpdateTeamPrivacy(teamIdStr, privacy, openInvite); err != nil {
		c.Err = err
		return
	}
...
func regenerateTeamInviteId(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	patchedTeam, err := c.App.RegenerateTeamInviteId(teamIdStr)
	if err != nil {
		c.Err = err
		return
	}
...
func deleteTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if team, err := c.App.GetTeam(teamIdStr); err == nil {
		model.AddEventParameterAuditableToAuditRec(auditRec, "team", team)
	}
	...
	if c.Params["permanent"].(bool) {
		if *c.App.Config().ServiceSettings.EnableAPITeamDeletion {
			err = c.App.PermanentDeleteTeamId(c.AppContext, teamIdStr)
		} else {
			...
			if usrErr == nil && user != nil && user.IsSystemAdmin() {
				...
				err = model.NewAppError("deleteTeam", "api.user.delete_team.not_enabled.for_admin.app_error", nil, "teamId="+teamIdStr, http.StatusUnauthorized)
			} else {
				err = model.NewAppError("deleteTeam", "api.user.delete_team.not_enabled.app_error", nil, "teamId="+teamIdStr, http.StatusUnauthorized)
			}
		}
	} else {
		err = c.App.SoftDeleteTeam(teamIdStr)
	}
...
func getTeamsForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if c.AppContext.Session().UserId != userIdStr && !c.App.SessionHasPermissionTo(*c.AppContext.Session(), model.PermissionSysconsoleReadUserManagementUsers) {
		c.SetPermissionError(model.PermissionSysconsoleReadUserManagementUsers)
		return
	}

	teams, appErr := c.App.GetTeamsForUser(userIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func getTeamsUnreadForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	unreadTeamsList, appErr := c.App.GetTeamsUnreadForUser(teamId, userIdStr, includeCollapsedThreads)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func getTeamMember(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	canSee, appErr := c.App.UserCanSeeOtherUser(c.AppContext, c.AppContext.Session().UserId, userIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
	...
	team, appErr := c.App.GetTeamMember(c.AppContext, teamIdStr, userIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func getTeamMembers(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	members, appErr := c.App.GetTeamMembers(teamIdStr, c.Params["page"].(int)*c.Params["per_page"].(int), c.Params["per_page"].(int), teamMembersGetOptions)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func getTeamMembersForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToUser(*c.AppContext.Session(), userIdStr) && !c.App.SessionHasPermissionTo(*c.AppContext.Session(), model.PermissionReadOtherUsersTeams) {
	...
	members, appErr := c.App.GetTeamMembersForUser(c.AppContext, userIdStr, "", true)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func getTeamMembersByIds(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	members, appErr := c.App.GetTeamMembersByIds(teamIdStr, userIDs, restrictions)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func addTeamMember(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if member.UserId == c.AppContext.Session().UserId {
		...
		canInviteGuests := c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionInviteGuest)
		if !canInviteGuests {
...
func addTeamMembers(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	team, appErr := c.App.GetTeam(teamIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
	...
	if !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionAddUserToTeam) {
		c.SetPermissionError(model.PermissionAddUserToTeam)
		return
	}
	...
	membersWithErrors, appErr := c.App.AddTeamMembers(c.AppContext, teamIdStr, userIDs, c.AppContext.Session().UserId, graceful)
...
func removeTeamMember(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if c.AppContext.Session().UserId != userIdStr {
		if !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionRemoveUserFromTeam) {
	...
	if team.IsGroupConstrained() && (userIdStr != c.AppContext.Session().UserId) && !user.IsBot {
		c.Err = model.NewAppError("removeTeamMember", "api.team.remove_member.group_constrained.app_error", nil, "", http.StatusBadRequest)
		return
	}

	if err := c.App.RemoveUserFromTeam(c.AppContext, teamIdStr, userIdStr, c.AppContext.Session().UserId); err != nil {
		c.Err = err
		return
	}
...
func getTeamUnread(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	unreadTeam, err := c.App.GetTeamUnread(teamIdStr, userIdStr)
	if err != nil {
		c.Err = err
		return
	}
...
func getTeamStats(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	stats, err := c.App.GetTeamStats(teamIdStr, restrictions)
	if err != nil {
		c.Err = err
		return
	}
...
func updateTeamMemberRoles(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionManageTeamRoles) {
		c.SetPermissionError(model.PermissionManageTeamRoles)
		return
	}

	teamMember, err := c.App.UpdateTeamMemberRoles(c.AppContext, teamIdStr, userIdStr, newRoles)
	if err != nil {
		c.Err = err
		return
	}
...
func updateTeamMemberSchemeRoles(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	teamMember, err := c.App.UpdateTeamMemberSchemeRoles(c.AppContext, teamIdStr, userIdStr, schemeRoles.SchemeGuest, schemeRoles.SchemeUser, schemeRoles.SchemeAdmin)
	if err != nil {
		c.Err = err
		return
	}
...
func getAllTeams(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if c.Params["exclude_policy_constrained"].(bool) {
	...
	offset := limit * c.Params["page"].(int)
...
func importTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionImportTeam) {
	...
	case "slack":
		var err *model.AppError
		if err, log = c.App.SlackImport(c.AppContext, fileData, fileSize, teamIdStr); err != nil {
			c.Err = err
...
func inviteUsersToTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if graceful {
		...
		var appErr *model.AppError
		if emailList != nil {
			invitesWithError, appErr = c.App.InviteNewUsersToTeamGracefully(c.AppContext, memberInvite, teamIdStr, c.AppContext.Session().UserId, "")
		}
		...
		appErr := c.App.InviteNewUsersToTeam(c.AppContext, emailList, teamIdStr, c.AppContext.Session().UserId)
		if appErr != nil {
			c.Err = appErr
			return
		}
...
func inviteGuestsToChannels(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(*c.AppContext.Session(), teamIdStr, model.PermissionInviteGuest) {
		c.SetPermissionError(model.PermissionInviteGuest)
		return
	}
	...
	if graceful {
		...
		if guestsInvite.Emails != nil {
			invitesWithError, appErr = c.App.InviteGuestsToChannelsGracefully(c.AppContext, teamIdStr, &guestsInvite, c.AppContext.Session().UserId, guestMagicLink)
		}
		...
		appErr := c.App.InviteGuestsToChannels(c.AppContext, teamIdStr, &guestsInvite, c.AppContext.Session().UserId, guestMagicLink)
		if appErr != nil {
			c.Err = appErr
			return
		}
...
func getInviteInfo(c *Context, w http.ResponseWriter, r *http.Request) {
	inviteId := c.RequireParam("invite_id", web.RequireString)
	if c.Err != nil {
		return
	}
	inviteIdStr := inviteId.(string)

	team, appErr := c.App.GetTeamByInviteId(inviteIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
	...
	if team.Type != model.TeamOpen {
		c.Err = model.NewAppError("getInviteInfo", "api.team.get_invite_info.not_open_team", nil, "id="+inviteIdStr, http.StatusForbidden)
		return
	}
...
func setTeamIcon(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := c.App.SetTeamIcon(c.AppContext, teamIdStr, imageData); err != nil {
		c.Err = err
		return
	}
...
func removeTeamIcon(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := c.App.RemoveTeamIcon(teamIdStr); err != nil {
		c.Err = err
		return
	}
...
func teamMembersMinusGroupMembers(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	groupIDsParam := groupIDsQueryParamRegex.ReplaceAllString(c.Params["group_ids"].(string), "")
	...
	groupIDs := []string{}
	for gid := range strings.SplitSeq(c.Params["group_ids"].(string), ",") {
