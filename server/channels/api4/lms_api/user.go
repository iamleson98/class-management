...
func createUser(c *api4.Context, w http.ResponseWriter, r *http.Request) {
	...
	createdUser, err := c.App.CreateUser(c.AppContext, user)
	if err != nil {
		c.Err = err
		return
	}

	// add user to according team
	roles := createdUser.GetRoles()
	var teamName = model.TeamUser
out:
	for _, role := range roles {
		switch role {
		case model.RoleLmsAdminRoleId, model.RoleLmsTeacherRoleId, model.RoleLmsAccountantRoleId, model.RoleLmsSuperAdminRoleId, model.RoleLmsCounselorRoleId, model.RoleLmsMarketingRoleId:
			teamName = model.TeamEmployee
			break out
		default:
			continue
		}
	...
	_, _, err = c.App.AddUserToTeam(c.AppContext, team.Id, createdUser.Id, c.AppContext.Session().UserId)
	if err != nil {
		c.Err = err
		return
	}
