...
func (api *API) InitOutgoingOAuthConnection() {
	api.BaseRoutes.OutgoingOAuthConnections.Method(http.MethodGet, "/", api.APISessionRequired(listOutgoingOAuthConnections))
	api.BaseRoutes.OutgoingOAuthConnections.Method(http.MethodPost, "/", api.APISessionRequired(createOutgoingOAuthConnection))
	api.BaseRoutes.OutgoingOAuthConnection.Method(http.MethodGet, "/", api.APISessionRequired(getOutgoingOAuthConnection))
	api.BaseRoutes.OutgoingOAuthConnection.Method(http.MethodPut, "/", api.APISessionRequired(updateOutgoingOAuthConnection))
	api.BaseRoutes.OutgoingOAuthConnection.Method(http.MethodDelete, "/", api.APISessionRequired(deleteOutgoingOAuthConnection))
	api.BaseRoutes.OutgoingOAuthConnections.Method(http.MethodPost, "/validate", api.APISessionRequired(validateOutgoingOAuthConnectionCredentials))
}
...
func updateOutgoingOAuthConnection(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	defer c.LogAuditRec(auditRec)
	model.AddEventParameterToAuditRec(auditRec, "outgoing_oauth_connection_id", outgoingConnectionIdStr)
	c.LogAudit("attempt")
	...
	if inputConnection.Id != outgoingConnectionIdStr {
		c.SetInvalidParam("id")
		return
	}

	currentConnection, err := service.GetConnection(c.AppContext, outgoingConnectionIdStr)
	if err != nil {
...
func deleteOutgoingOAuthConnection(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := service.DeleteConnection(c.AppContext, outgoingConnectionIdStr); err != nil {
		c.Err = model.NewAppError(whereOutgoingOAuthConnection, "api.context.outgoing_oauth_connection.delete_connection.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
		return
	}
