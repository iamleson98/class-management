...
func getCategoriesForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	categories, appErr := c.App.GetSidebarCategoriesForTeamForUser(c.AppContext, userIdStr, teamIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func createCategoryForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || userIdStr != categoryCreateRequest.UserId || teamIdStr != categoryCreateRequest.TeamId {
		c.SetInvalidParamWithErr("category", err)
		return
	}

	if appErr := validateSidebarCategory(c, teamIdStr, userIdStr, &categoryCreateRequest); appErr != nil {
		c.Err = appErr
		return
	}

	category, appErr := c.App.CreateSidebarCategory(c.AppContext, userIdStr, teamIdStr, &categoryCreateRequest)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func getCategoryOrderForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	order, appErr := c.App.GetSidebarCategoryOrder(c.AppContext, userIdStr, teamIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func updateCategoryOrderForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	for _, categoryId := range categoryOrder {
		if !c.App.SessionHasPermissionToCategory(c.AppContext, *c.AppContext.Session(), userIdStr, teamIdStr, categoryId) {
			c.SetInvalidParam("category")
			return
		}
	}

	appErr := c.App.UpdateSidebarCategoryOrder(c.AppContext, userIdStr, teamIdStr, categoryOrder)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func getCategoryForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	categories, appErr := c.App.GetSidebarCategory(c.AppContext, categoryIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func updateCategoriesForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	for _, category := range categoriesUpdateRequest {
		if !c.App.SessionHasPermissionToCategory(c.AppContext, *c.AppContext.Session(), userIdStr, teamIdStr, category.Id) {
			c.SetInvalidParam("category")
			return
		}
	}

	if appErr := validateSidebarCategories(c, teamIdStr, userIdStr, categoriesUpdateRequest); appErr != nil {
		c.Err = appErr
		return
	}

	categories, appErr := c.App.UpdateSidebarCategories(c.AppContext, userIdStr, teamIdStr, categoriesUpdateRequest)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func updateCategoryForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || categoryUpdateRequest.TeamId != teamIdStr || categoryUpdateRequest.UserId != userIdStr {
		c.SetInvalidParamWithErr("category", err)
		return
	}

	if appErr := validateSidebarCategory(c, teamIdStr, userIdStr, &categoryUpdateRequest); appErr != nil {
		c.Err = appErr
		return
	}

	categoryUpdateRequest.Id = categoryIdStr

	categories, appErr := c.App.UpdateSidebarCategories(c.AppContext, userIdStr, teamIdStr, []*model.SidebarCategoryWithChannels{&categoryUpdateRequest})
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func deleteCategoryForTeamForUser(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	appErr := c.App.DeleteSidebarCategory(c.AppContext, userIdStr, teamIdStr, categoryIdStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
