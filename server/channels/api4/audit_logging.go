...
func (api *API) InitAuditLogging() {
	api.BaseRoutes.AuditLogs.Method(http.MethodPost, "/certificate", api.APISessionRequired(addAuditLogCertificate))
	api.BaseRoutes.AuditLogs.Method(http.MethodDelete, "/certificate", api.APISessionRequired(removeAuditLogCertificate))
}
