...
func (api *API) InitUpload() {
	api.BaseRoutes.Uploads.Method(http.MethodPost, "/", api.APISessionRequired(createUpload, handlerParamFileAPI))
	api.BaseRoutes.Upload.Method(http.MethodGet, "/", api.APISessionRequired(getUpload))
	api.BaseRoutes.Upload.Method(http.MethodPost, "/", api.APISessionRequired(uploadData, handlerParamFileAPI))
}
