...
func (api *API) InitAccessControlPolicy() {
	...
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodPut, "/", api.APISessionRequired(createAccessControlPolicy))
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodPost, "/search", api.APISessionRequired(searchAccessControlPolicies))
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodPut, "/activate", api.APISessionRequired(setActiveStatus))
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodPost, "/cel/check", api.APISessionRequired(checkExpression))
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodPost, "/cel/test", api.APISessionRequired(testExpression))
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodPost, "/cel/validate_requester", api.APISessionRequired(validateExpressionAgainstRequester))
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodGet, "/cel/autocomplete/fields", api.APISessionRequired(getFieldsAutocomplete))
	api.BaseRoutes.AccessControlPolicies.Method(http.MethodPost, "/cel/visual_ast", api.APISessionRequired(convertToVisualAST))
	api.BaseRoutes.AccessControlPolicy.Method(http.MethodGet, "/", api.APISessionRequired(getAccessControlPolicy))
	api.BaseRoutes.AccessControlPolicy.Method(http.MethodDelete, "/", api.APISessionRequired(deleteAccessControlPolicy))
	api.BaseRoutes.AccessControlPolicy.Method(http.MethodGet, "/activate", api.APISessionRequired(updateActiveStatus))
	api.BaseRoutes.AccessControlPolicy.Method(http.MethodPost, "/assign", api.APISessionRequired(assignAccessPolicy))
	api.BaseRoutes.AccessControlPolicy.Method(http.MethodDelete, "/unassign", api.APISessionRequired(unassignAccessPolicy))
	api.BaseRoutes.AccessControlPolicy.Method(http.MethodGet, "/resources/channels", api.APISessionRequired(getChannelsForAccessControlPolicy))
	api.BaseRoutes.AccessControlPolicy.Method(http.MethodPost, "/resources/channels/search", api.APISessionRequired(searchChannelsForAccessControlPolicy))
}
...
func deleteAccessControlPolicy(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	defer c.LogAuditRec(auditRec)
	model.AddEventParameterToAuditRec(auditRec, "id", policyIDStr)

	// Check if user has system admin permission OR channel-specific permission
	hasManageSystemPermission := c.App.SessionHasPermissionTo(*c.AppContext.Session(), model.PermissionManageSystem)
	if !hasManageSystemPermission {
		// For non-system admins, validate policy access permission
		if appErr := c.App.ValidateAccessControlPolicyPermission(c.AppContext, c.AppContext.Session().UserId, policyIDStr); appErr != nil {
			c.SetPermissionError(model.PermissionManageSystem)
			return
		}
	}

	appErr := c.App.DeleteAccessControlPolicy(c.AppContext, policyIDStr)
	if appErr != nil {
		c.Err = appErr
		return
	}
...
func updateActiveStatus(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !hasManageSystemPermission {
		// For non-system admins, validate policy access permission
		if appErr := c.App.ValidateAccessControlPolicyPermission(c.AppContext, c.AppContext.Session().UserId, pollicyIdStr); appErr != nil {
			c.SetPermissionError(model.PermissionManageSystem)
			return
		}
	}
	...
	defer c.LogAuditRec(auditRec)
	model.AddEventParameterToAuditRec(auditRec, "id", pollicyIdStr)
	...
	updates := []model.AccessControlPolicyActiveUpdate{
		{ID: pollicyIdStr, Active: activeBool},
	}
...
func assignAccessPolicy(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if len(assignments.ChannelIds) != 0 {
		_, appErr := c.App.AssignAccessControlPolicyToChannels(c.AppContext, policyIDStr, assignments.ChannelIds)
		if appErr != nil {
			c.Err = appErr
			return
		}
...
func unassignAccessPolicy(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if len(assignments.ChannelIds) != 0 {
		appErr := c.App.UnassignPoliciesFromChannels(c.AppContext, pollicyIdStr, assignments.ChannelIds)
		if appErr != nil {
			c.Err = appErr
			return
		}
...
func getChannelsForAccessControlPolicy(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	channels, total, appErr := c.App.GetChannelsForPolicy(c.AppContext, policyIDStr, model.AccessControlPolicyCursor{
		ID: afterID,
	}, limit)
	if appErr != nil {
		c.Err = appErr
...
func searchChannelsForAccessControlPolicy(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	opts := model.ChannelSearchOpts{
		...
		ParentAccessControlPolicyId: policyIDStr,
	}
