...
export default function EnableSimulcast(props: CustomComponentProps) {
    const {formatMessage} = useIntl();
    const config = useSelector(callsConfig);
    const overrides = useSelector(callsConfigEnvOverrides);
    const overridden = 'EnableSimulcast' in overrides;
    ...
    let checked;
    if (overridden) {
        checked = config.EnableSimulcast;
    } else {
    ...
    return (
        ...
        >
            <label className={'control-label ' + leftCol}>
                {formatMessage({defaultMessage: 'Enable simulcast for screen sharing (Experimental)'})}
            </label>
