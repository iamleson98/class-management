...
const UDPServerPort = (props: CustomComponentProps) => {
    ...
    const overridden = 'UDPServerPort' in overrides;
    const helpText = useHelptext(formatMessage({defaultMessage: 'The UDP port the RTC server will listen on.'}));
    ...
    const value = overridden ? config.UDPServerPort : props.value;
            ...
            <div className={'control-label ' + leftCol}>
                <LabelRow>
                    <label
                        ...
                        htmlFor={props.id}
                    >
                        {formatMessage({defaultMessage: 'RTC Server Port (UDP)'})}
                    </label>
                </LabelRow>
            </div>
