...
func (p *Plugin) publishWebSocketEvent(ev string, data map[string]any, broadcast *WebSocketBroadcast) {
	botID := p.getBotID()
...
func (p *Plugin) handleClientMessageTypeScreen(us *session, msg clientMessage, handlerID string) error {
	...
	p.publishWebSocketEvent(wsMsgType, map[string]any{
		"userID":     us.userID,
...
func (p *Plugin) wsWriter() {
	for {
		...
		case msg, ok := <-p.rtcServer.ReceiveCh():
			...
			p.publishWebSocketEvent(wsEventSignal, map[string]any{
...
func (p *Plugin) handleJoin(userID, connID, authSessionID string, joinData callsJoinData) (retErr error) {
	...
	addSessionToCall := func(state *callState) *callState {
		...
		if err != nil {
			...
			p.publishWebSocketEvent(wsEventError, map[string]any{
				"data":   err.Error(),
				"connID": connID,
			...
			p.publishWebSocketEvent(wsEventCallStart, map[string]any{
				"id":        state.Call.ID,
				"channelID": channelID,
		...
		p.publishWebSocketEvent(wsEventJoin, map[string]any{
			"connID": connID,
		}, &WebSocketBroadcast{ConnectionID: connID, ReliableClusterSend: true})
		...
		if userID == p.getBotID() && state.Recording != nil {
			p.publishWebSocketEvent(wsEventCallJobState, map[string]any{
				"callID":   channelID,
		...
		if err != nil {
			...
			p.publishWebSocketEvent(wsEventCallState, map[string]any{
...
func (p *Plugin) handleCaptionMessage(callID, channelID, captionFromSessionID, text string, newAudioLenMs float64) error {
	...
	p.publishWebSocketEvent(wsEventCaption, map[string]any{
		"channel_id": channelID,
